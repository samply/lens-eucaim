...
} from "@samply/lens-components/result-table";
...
} from "@samply/lens-components/result-summary-bar";
...
export class MainComponent {
  ...
  private ageSorters: Array<(a: { key: string, population: number }, b: { key: string, population: number }) => number>
    = [(a, b) => { return parseInt(a.key) - parseInt(b.key) }]
  ...
  private diagnosisAggregators: Array<(values: Array<{ key: string, population: number }>) => Array<{ key: string, population: number }>>
    = [
      // Filter the Input
      (values) => {
      ...
      (values) => {
        if (values.every(value => value.key.indexOf(values[0].key) !== -1)) {
          ...
          let adjustedIcdCodes = values.map(value => {
            return { key: value.key.substring(0, 4), population: value.population }
          })
          // 2) Aggregate the populations
          let aggregatedResults: Array<{ key: string, population: number }> = [];
          adjustedIcdCodes.forEach(value => {
            let index = aggregatedResults.findIndex(icdCode => icdCode.key === value.key)
            if (index != -1) {
